...
for (const pattern of [
  ...
  "function workflowPurchaseLifecycleSource",
  "function workflowPurchaseCreatorName",
  "function workflowPurchaseVersionLabel",
  "function workflowPurchaseLastRunText",
...
for (const pattern of [
  ...
  "apiPost<Work>(`/works/${work.id}/favorite`",
  "apiDelete<Work>(`/works/${work.id}/favorite`",
...
for (const pattern of [
  ...
  'onRequireAuthAction({ type: "gallery-favorite", workId: work.id })',
  'pendingAction.type !== "gallery-download" && pendingAction.type !== "gallery-rerun" && pendingAction.type !== "gallery-favorite"',
  ...
  'pendingAction.type !== "share-download" && pendingAction.type !== "share-rerun" && pendingAction.type !== "share-favorite"',
  'requireCaseAuth({ type: "case-rerun", caseId: item.id }',
  'requireCaseAuth({ type: "case-download", caseId: item.id }',
  'requireCaseAuth({ type: "case-favorite", caseId: item.id }',
  'requireCaseAuth({ type: "case-workflow", caseId: item.id }',
  'apiGet<PageData<CaseContent>>(`/case-contents?${params.toString()}`',
  'apiGet<CaseContent>(`/case-contents/${item.id}`',
  'apiPost<GenerationSubmitResult>("/generation-tasks"',
  'apiPost<CaseContent>(`/case-contents/${item.id}/favorite`',
  'apiDelete<CaseContent>(`/case-contents/${item.id}/favorite`',
