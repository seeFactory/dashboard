...
assert.ok(files.length, "Dashboard dist must contain build artifacts before verifying production API base.");
...
for (const file of files) {
  ...
  for (const pattern of forbiddenPatterns) {
    assert.ok(!text.includes(pattern), `Dashboard production artifact ${file} must not include ${pattern}.`);
  }
}
